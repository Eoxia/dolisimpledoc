...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create' && $action != 'letterpresend'))) {
	...
	if (empty($reshook)) {
		...
		if ($permissiontoadd) {
			print '<a class="'. ($object->status == 0 ? 'butAction"' : 'butActionRefused classfortooltip"  title="'. $langs->trans('CantEditAlreadySigned').'"') . ' id="actionButtonEdit" href="' . $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=edit&token='.newToken().'">' . $langs->trans("Modify") . '</a>' . "\n";
			...
			print '<a class="'. ($object->status == 3 ? 'butAction"' : 'butActionRefused classfortooltip" title="'. $langs->trans('MustBeSentBeforeAcknowledgementReceipt').'"') .'"  id="actionButtonUploadReceiptAcknowledgement" href="' . $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=uploadAcknowledgementReceipt&token='.newToken().'">' . $langs->trans("UploadAcknowledgementReceipt") . '</a>' . "\n";
		} else {
