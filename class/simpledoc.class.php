 ...
 * Class for SimpleDoc
 */
class SimpleDoc extends CommonObject
{
	/**
	...
	public $fields=array(
		...
		'note' => array('type'=>'html', 'label'=>'NotePublic', 'enabled'=>'1', 'position'=>61, 'notnull'=>0, 'visible'=>0,)
		);
	public $rowid;
	public $ref;
	public $fk_soc;
	...
	//  */
	// public $fk_element = 'fk_simpledoc';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'SimpleDocline';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('SIMPLEDOC_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'simpledoc/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'simpledoc/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'SIMPLEDOC_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'SIMPLEDOC_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'SIMPLEDOC_REOPEN');
	}
	...
	public function getLinesArray()
	{
		...
		$objectline = new SimpleDocLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_simpledoc = '.$this->id));
...
/**
 * Class SimpleDocLine. You can also remove this and generate a CRUD class for lines objects.
 */
class SimpleDocLine extends CommonObjectLine
{
	// To complete with content of an object SimpleDocLine
	// We should have a field rowid, fk_simpledoc and position
